	...
	public static void main(String args[]) {
		...
		for(int i=0;i<fh.ncols;i++) {
			for(int j=0;j<fh.nrows;j++) {
				if(cubic[i][j].value<=stg[0] && cubic[i][j].value>0) {
					cubic[i][j].stage=1;
				}
				if(cubic[i][j].value<=stg[1] && cubic[i][j].value>stg[0]) {
					cubic[i][j].stage=2;
				}
				if(cubic[i][j].value>stg[1]) {
					cubic[i][j].stage=3;
				}								
			}
		}
		...
		try (BufferedWriter bw = new BufferedWriter(new FileWriter("./result/seeds.rb"))){
			...
			for(int m=0; m<poly.length; m++) {
				...
				String line = "Rectangle.create(hazard_id:"+args[2]+",anchor_id:"+args[5]+",iteration:"+args[3]+", zone_id:"+zone_id+", rank:"+poly[m].level;
